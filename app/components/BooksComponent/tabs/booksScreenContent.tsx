...
import { recommended, newBooks, NewBooks, fictionBooks, FictionBooks } from '../data/booksContent'
...
export const New: React.FC = () =>{
    const renderItem = ({ item }: { item: NewBooks }) => (
...
export const Fiction: React.FC = () =>{
    const renderItem = ({ item }: { item: FictionBooks }) => (
    <YStack style={newStyle.imageContainer}>
      <Image source={{ uri: item.cover }} style={newStyle.image}/>
      <Text numberOfLines={1} style={newStyle.title}>{item.title}</Text>
    </YStack>
    ...
    return(
        <TamaguiProvider config={config}>
            <YStack style={styles.container}>
                <YStack style={newStyle.container}>
                    <Text style={newStyle.header}>Fiction</Text>
                    <YStack>
                        <FlatList
                            horizontal
                            data={fictionBooks}
