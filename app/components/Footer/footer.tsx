...
export default function Footer() {
  ...
  return (
    <TamaguiProvider config={config}>
        ...
        <Button onPress={() => router.push('/components/HomeComponent/homeScreen')} style={styles.button}>
            <Icons.House size="$1.5"  color={isActive('/homeScreen') ? '#FFF' : '#D88632'} strokeWidth={isActive('/homeScreen') ? 2 : 1.5} />
        </Button>
        <Button onPress={() => router.push('/components/ExflixComponent/exflixScreen')} style={styles.button}>
            <Icons.Film size="$1.5" color={isActive('/exflixScreen') ? '#FFF' : '#D88632'} strokeWidth={isActive('/exflixScrenn') ? 2 : 1.5}/>
        </Button>
        <Button onPress={() => router.push('/components/CreatePostComponent/createPostScreen')} style={styles.button}>
            <Icons.Plus size="$1.5" color={isActive('/home') ? '#FFF' : '#D88632'} strokeWidth={isActive('/home') ? 2 : 1.5}/>
        </Button>
        <Button onPress={() => router.push('/components/ShopComponent/shoppingProductScreen')} style={styles.button}>
            <Icons.Store size="$1.5" color={isActive('/shop_products') ? '#FFF' : '#D88632'} strokeWidth={isActive('/shop_products') ? 2 : 1.5}/>
        </Button>
        <Button onPress={() => router.push('/components/MenuComponent/menuScreen')} style={styles.button}>
